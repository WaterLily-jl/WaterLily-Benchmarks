...
- **Δ/Noise**: Δ in units of its own scatter, `Δ / sqrt(Noise_row² + Noise_ref²)`, since Δ is the difference of two noisy rows. Below about 1 the Δ is indistinguishable from scatter; believe it from about 2 to 3. It is only as reliable as `Noise`: with `Reps = 1` on a machine whose state changes between processes it can be large and wrong.
